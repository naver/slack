...
def train_and_eval(rank, worldsize, tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='last', save_path=None, only_eval=False, resume=None):
    if not reporter:
    ...
    trainsampler, trainloader, validloader, testloader_, testtrainloader_, dataset_info = get_dataloaders(C.get()['dataset'], C.get()['batch'], dataroot, test_ratio, split_idx=cv_fold, distributed=worldsize>1, started_with_spawn=C.get()['started_with_spawn'], summary_writer=writers[0], resume=resume)
...
def parse_args():
    ...
    parser.add_argument('--optimizer_decay', type=float, required=False)
    parser.add_argument('--optimizer_nesterov', action='store_true', required=False)
...
def spawn_process(global_rank, worldsize, port_suffix, args, config_path=None, communicate_results_with_queue=None, local_rank=None,):
    ...
    assert 'seed' in C.get() or args.seed is not None
    if 'seed' in C.get() or args.seed is not None:
        seed = C.get()['seed'] if 'seed' in C.get() else args.seed
    ...
    import time
    t = time.time()
    result = train_and_eval(local_rank, worldsize, args.tag, args.dataroot, test_ratio=args.cv_ratio, cv_fold=args.cv, save_path=args.save, only_eval=args.only_eval, metric='last', resume=args.resume)
    elapsed = time.time() - t
    print('done')
